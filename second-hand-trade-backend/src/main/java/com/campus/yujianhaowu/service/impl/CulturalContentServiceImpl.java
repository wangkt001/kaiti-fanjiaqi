...
import com.campus.yujianhaowu.util.ImageUtil;
...
public class CulturalContentServiceImpl implements CulturalContentService {
    ...
    private CulturalContent createContent(String title, String summary, String category,
        ...
        try {
            // 如果是本地图片路径
            if (coverImageUrl.startsWith("/") || coverImageUrl.startsWith("C:") || coverImageUrl.startsWith("D:")) {
                File imageFile = new File(coverImageUrl);
                if (imageFile.exists()) {
                    String base64 = ImageUtil.imageToBase64(imageFile);
                    if (base64 != null) {
                        contentEntity.setCoverImageBase64(base64);
                        log.info("图片已转 Base64: {}", title);
                    }
                }
            }
            // 如果是网络图片，可以下载后转 Base64（这里暂不实现）
        } catch (Exception e) {
            log.warn("图片转 Base64 失败：{} - {}", title, e.getMessage());
        }
